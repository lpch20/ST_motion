...
export class EngagementModel extends MainModel {
  ...
  add(
    ...
    con: any,
    callBack: (r: ResultWithData<Customer[]>) => void
  ): void {
    let QUERY: string = `INSERT INTO engagement(idEvent,paymentPromiseDate,numberOfFees,amountFees,agreedDebt,initialDelivery,idBranchOffice,rate,portfolio)
                            VALUES(?,?,?,?,?,?,?,?,?)`;
    let values = [
      engagement.idEvent,
      super.formatDateTimeSql(engagement.paymentPromiseDate),
      engagement.numberOfFees,
      engagement.amountFees,
      engagement.agreedDebt,
      engagement.initialDelivery,
      engagement.idBranchOffice,
      engagement.rate,
  ...
  getEngagementByAgentByDate(
    ...
    var mainThis = this;
    let QUERY: string =
      "SELECT * FROM customer_events ce INNER JOIN engagement e ON ce.id = e.idEvent WHERE ce.idUser = ? AND ce.date BETWEEN ? AND ?";
    con.query(
      QUERY,
      [idAgent, dateFrom, dateTo],
  ...
  getEngagementByDate(
    ...
    var mainThis = this;
    let QUERY: string =
      "SELECT * FROM customer_events ce INNER JOIN engagement e ON ce.id = e.idEvent WHERE ce.date BETWEEN ? AND ? ORDER BY ce.date DESC";
  ...
  getCustomersByDateFromEngagements(
    dateFrom: Date,
    dateTo: Date,
    con: any,
    callBack: (r: ResultWithData<Customer[]>) => void
  ...
  getCustomersByUserByDateFromEngagements(
    userId: number,
    dateFrom: Date,
    dateTo: Date,
    con: any,
  ...
  getEngagementByCustomerByDate(
    ...
    var mainThis = this;
    let QUERY: string =
      "SELECT * FROM customer_events ce INNER JOIN engagement e ON ce.id = e.idEvent ";
    QUERY +=
      " INNER JOIN customer c ON ce.idCustomer = c.id WHERE ce.idCustomer = ? AND ce.date BETWEEN ? AND ? ORDER BY ce.date DESC";
    let queryPerformed = con.query(
      QUERY,
      [idCustomer, dateFrom, dateTo],
      function (err: any, result: any[]) {
        if (!!err) {
          mainThis.errorModel(con, err, callBack);
        } else {
