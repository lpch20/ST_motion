...
DBUsers.prototype.login = function (user, password,changePass = false, loginCallback) {
    var userDataBuilder = require('../datatypes/User');
    var loginResponseBuilder = require('../datatypes/LoginResponse');
    var proyection = 'id,password,user_name,active,rol_id,numberOfAttempts,requireNewPassword';
    ...
    this.masterDBConnection.connection.query('SELECT ' + proyection + ' FROM users WHERE user_name = ? ',
        ...
        function (err, rows, fields) {
            if (err) {
                ...
                if (rows && rows.length > 0 && rows[0].numberOfAttempts > 5) {
                    loginCallback(null, new loginResponseBuilder(null, false, 'user blocked', false, null));
                    return;
                } else {
                    if ((!rows) || (!rows[0]) || (sha1(password.trim()) !== rows[0].password)) {
                        mdbcon.query('UPDATE users  SET numberOfAttempts = (numberOfAttempts + 1) WHERE user_name = ? ',
                            [user.trim()],
                            function (err, rows, fields) {
                                if (err) {
                                    console.error(now.toString(), "DBUsers - segundo if => ", err);
                                    loginCallback(err);
                                    return;
                                } else {
                        ...
                        if (parseInt(rows[0].active) === 0) {
                            ...
                            if (rows[0].requireNewPassword && !changePass) {
                                loginCallback(null, new loginResponseBuilder(null, false, 'Require new password', false, null));
                                return;
                            } else {
                                mdbcon.query('UPDATE users  SET numberOfAttempts = 0 WHERE user_name = ? ',
                                    [user.trim()],
                                    function (err, rowsNumberOfAttempts, fields) {
                                        if (err) {
                                            console.error(now.toString(), "DBUsers - segundo if => ", err);
                                            loginCallback(err);
                                            return;
