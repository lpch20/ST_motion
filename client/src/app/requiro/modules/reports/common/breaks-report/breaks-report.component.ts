  ...
  implements OnInit, AfterViewInit
{
  ...
  constructor(private breakService: BreakService, usersService: UsersService) {
    super(usersService);
  }

  ngOnInit() {
    this.displayedColumns = this.supervisorView
      ? ["date", "idUser", "idTypeBreak", "init"]
      : ["date", "idTypeBreak", "init"];
    this.breakService.getBreakTypes().subscribe((response) => {
  ...
  exportCSV(): void {
    ...
    for (let i = 0; i < this.totalTimeBreakByType.length; i++) {
      data.push({
        date: this.totalTimeBreakByType[i].type,
        agentName: "",
        agent: "",
        motivo: "",
        type: "",
      ...
      data.push({
        date: this.totalTimeBreakByType[i].time,
        agentName: "",
        agent: "",
        motivo: "",
        type: "",
  ...
  getBreakTypeNameById(idBreakType: number): string {
    if (this.breakTypes) {
      const breakType: BreakType = this.breakTypes.filter(
        (u) => u.id === idBreakType
      )[0];
  ...
  searchBreaks(): void {
    ...
    if (this.supervisorView) {
      this.breakService
        .getBreakByDate(this.selectedUserId, this.dateFrom, this.dateTo)
        .subscribe(
          (response) => {
            if (response.result === ResultCode.OK) {
              this.breaks = response.data;
              this.dataSource.data = response.data;
              this.totalTimeBreakByType =
      ...
      this.breakService
        .getBreakByCurrentUserByDate(this.dateFrom, this.dateTo)
        .subscribe(
          (response) => {
            if (response.result === ResultCode.OK) {
              this.breaks = response.data;
              this.dataSource.data = response.data;
              this.totalTimeBreakByType = this.calculateTotalTimeBreakByType(
                response.data
              );
  ...
  private calculateTotalTimeBreakByType(data): BreakByType[] {
    ...
    for (let i = 0; i < this.breakTypes.length; i++) {
      const breaksByType: Array<any> = data.filter(
        (b) => b.idTypeBreak === this.breakTypes[i].id
      );
      if (breaksByType.length > 0) {
        totalTimeBreakByType.push({
          type: this.breakTypes[i].label,
          time: this.totalDuration(breaksByType),
        });
      }
    }
