...
export class CustomerEventReportComponent extends UserDateReport implements OnInit, AfterViewInit {
    ...
    selectedUserId = 0;
    displayedColumns = ['date', 'eventType', 'user', 'ci', 'customer', 'departament', 'city', 'address', 'phone', 'message'];
    // 'campaign', 
    ...
    private getCities(): void {
        this.departamentService.getAllCities()
            .subscribe(
                c => {
                    this.cities = c.data;
                },
    ...
    private getCityById(id: number): { id: number, name: string, idDepartment: number } {
        return this.cities ? this.cities.filter(c => c.id === id)[0] : null;
    }

    private getDepartmentById(id: number): { id: number, name: string } {
        return this.departaments ? this.departaments.filter(c => c.id === id)[0] : null;
    }

    public getCampaignByCustomerId(idCustomer: number): string {
        if (this.customersCampaign && this.campaigns) {
            const customerCampaign = this.customersCampaign.find(cc => cc.idCustomer === idCustomer);
            if (customerCampaign) {
                const campaign = this.campaigns.find(c => c.id === customerCampaign.idCampaign);
                if (campaign) {
    ...
    searchEvents(): void {
        ...
        if (this.supervisorView) {
            this.eventService.getEventByDate(this.selectedUserId, this.dateFrom, this.dateTo)
                .subscribe(
                    response => {
                        if (response.result === ResultCode.OK) {
                            ...
                            if (response.data && response.data.length > 0) {
                                const uniqueIdCustomers = Array.from(new Set(response.data.map(c => c.idCustomer)));
                                this.customersService.getCustomersCampaign(uniqueIdCustomers)
                                    .subscribe(
            ...
            this.eventService.getEventByCurrentUserByDate(this.dateFrom, this.dateTo)
                .subscribe(
                    response => {
                        if (response.result === ResultCode.OK) {
                            this.eventsCustomers = response.data;
    ...
    changeEventType(id: number): void {
        if (id === 0) {
            ...
            this.dataSource.data = this.eventsCustomers.filter(e => e.eventType == id);
        }
    }
    ...
    exportCSV(): void {
        ...
        const data: { agent, campaign, ci, idPay, result, tel, date, cliente, department, city, address, message, action, portfolio }[] =
            new Array<{ agent, campaign, ci, idPay, result, tel, date, cliente, department, city, address, message, action, portfolio }>();

        const item = {
            agent: 'Agente', campaign: 'Campaña', date: 'Fecha', result: 'Resultado', ci: 'CI', idPay: 'idPago', cliente: 'Cliente',
            tel: 'Telefono', department: 'Departamento', city: 'Localidad', address: 'Dirección', action: 'Acción', message: 'Observación', portfolio: 'cartera'
        };
        data.push(item);
        ...
        for (let i = 0; i < this.eventsCustomers.length; i++) {
            ...
            if (line) {
                ...
                data.push({
                    ...
                    campaign: this.campaigns.filter(c => c.id === this.customersCampaign.filter(cc => cc.idCustomer === line.idCustomer)[0].idCampaign)[0].name,
                    ...
                    tel: line.phone,
                    department: this.getDepartmentById(line.idDepartment) ? this.getDepartmentById(line.idDepartment).name : '----',
                    city: this.getCityById(line.idCity) ? this.getCityById(line.idCity).name : '----',
